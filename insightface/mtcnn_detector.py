...
#from impl.helper import nms, adjust_input, generate_bbox, detect_first_stage_warpper
...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        # sym, arg_params, aux_params = mx.model.load_checkpoint('/akshat/models/arcface/mtcnn-model/det1', 1)
        # self.RNet = mx.mod.Module(symbol = sym, context=ctx)
        # self.RNet.bind(data_shapes = [('data', [1,3,300,300])])
        # self.RNet.set_params(arg_params, aux_params)

        # sym = mx.sym.load_json(open('/akshat/models/arcface/mtcnn-model/det2-symbol.json', 'r').read())
        # self.RNet = gluon.nn.SymbolBlock(outputs=sym, inputs=mx.sym.var('data'))
        # self.RNet.load_params('/akshat/models/arcface/mtcnn-model/det2-0001.params', ctx=ctx)
    ...
    def detect_face(self, img, det_type=0):
        ...
        ctx = self.ctx
        models = ['det1', 'det2', 'det3','det4']
        models = [ os.path.join('./models/mtcnn-model/', f) for f in models]
        ...
        for i in range(num_box):
            ...
            input_buf[i, :, :, :] = adjust_input(cv2.resize(tmp, (48, 48)))
        #-------------------------------------------------------------------------------
        # self.ONet = mx.model.FeedForward.load(os.path.join('/akshat/models/arcface/mtcnn-model/', 'det3'), 1, ctx=mx.cpu())
        output = self.ONet.predict(input_buf)
